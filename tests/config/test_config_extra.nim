...
suite "config: known-good lookup with normalized pretty names":
  ...
  test "knownGoodWireModel repairs listed-but-unserved variants":
    ...
    # Distinct models and unknown ids are not rewritten. kimi-for-coding
    # is a listed kimicode combo, so it resolves to itself, not ""; only
    # off-table ids (the retired kimi-k2.6 direct row) stay empty.
    check knownGoodWireModel("kimicode", "kimi-for-coding") == "kimi-for-coding"

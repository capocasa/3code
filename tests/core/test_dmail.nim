...
suite "checkpoint marker display stripping":
  test "drops a leading marker line":
    check stripCheckpointMarkers("[checkpoint 3]\nhello") == "hello"

  test "drops several echoed marker lines":
    # Marker lines drop; the blank lines between them are paragraph
    # breaks and stay (the live stream's trailing-blank suppression
    # handles the tail).
    check stripCheckpointMarkers("[checkpoint 14]\n\n[checkpoint 14]\n\n[checkpoint 14]\n") == "\n\n"

  test "keeps prose, drops echoed markers":
    check stripCheckpointMarkers("[checkpoint 1]\nDone. [checkpoint 2] More.") ==
      "Done.  More."

  test "leaves marker-free content untouched":
    check stripCheckpointMarkers("plain text\nmore text") == "plain text\nmore text"

...
proc toolCallToAction*(family, name: string, args: JsonNode): Action =
  ...
  case family
  of "glm", "qwen", "deepseek", "minimax", "longcat", "hy", "inkling", "laguna", "kimi", "gpt", "grok", "mimo", "ling", "0xalpha", "nemotron", "mistral", "claude", "other": dispatchGlmOrQwen(family, name, args)
...
proc runAction*(act: Action, cache: ReadCache = nil): tuple[output: string, code: int, diff: string] =
  ...
  of akWebSearch:
    ...
    try:
      let hits = webSearch(act.body, activeSearchKey, activeSearchEngine)

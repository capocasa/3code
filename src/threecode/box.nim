...
## no separate procbox binary to find or bundle. The box process forks,
## setsid()s, applies the OS-native restriction (Landlock/Seatbelt/ACL via
## the `procbox` library) and exec()s the target; children inherit the
## domain.
##
## Arg parsing mirrors procbox's CLI verbatim. The only divergence is the
## subcommand name: procbox's is `restrict`, here `restrict` follows `box`.
